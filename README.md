...
| **V2** | Prophet+XGBoost ensemble, historical residual std, momentum | **77.4%** (real holdout) |
...
**Real holdout validation (2026-04-18 → 04-24, 12 ports, 84 predictions):**
...
### Tab 1: Port Intelligence
- **Sidebar** (visible only on this tab): Most loaded ports list, model selector (Prophet/ARIMA/XGBoost)
- **CongestionHero:** Score gauge, trend, % vs 90-day normal
- **7-Day Outlook:** Forecasted congestion per day (UNVERIFIED badge when AIS coverage unavailable)
...
- **SupplyChainRiskCard:** Upstream chokepoints with disruption scores, transit lag days, and impact notes

### Tab 2: Live Vessels
- **VesselMap** (Leaflet): Real-time AIS vessel positions in US waters
- **AIS-coverage filter:** Only ports with reliable AIS coverage are shown (inland dark ports excluded)
- **Port markers:** Color-coded by congestion tier (red/amber/green) with sonar pulse animations
- **Vessel icons:** SVG shapes per vessel type (diamond=Cargo, triangle=Tanker, square=Passenger, etc.)
- **Clustering:** Vessels group into teal numbered bubbles at low zoom; uncluster at zoom ≥ 10
- **Tooltips:** Show congestion score, moored count, at-anchor count, incoming count (all from AIS)
- **Vessel filtering:** By type, nav status, congestion level

### Tab 3: AI Advisor
- Briefing cards — 3 most notable ports auto-generated by LLM at startup
- Chat interface with suggested starter questions and port comparison
